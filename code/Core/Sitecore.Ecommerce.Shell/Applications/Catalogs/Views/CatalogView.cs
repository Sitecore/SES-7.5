...
namespace Sitecore.Ecommerce.Shell.Applications.Catalogs.Views
{
  ...
  using ChecklistItem = Models.ChecklistItem;
  ...
  public class CatalogView : XamlMainControl, ICatalogView, IHasCommandContext
  {
    ...
    public StringCollection SelectedRowsId
    {
      get
      {
        return ViewState["SelectedRowsID"] as StringCollection;
      }

      set
      {
        ViewState["SelectedRowsID"] = value;
      }
    }
    ...
    public virtual ItemUri CurrentItemUri
    {
      get
      {
        ...
        string language = WebUtil.GetQueryString("language");
    ...
    /// <summary>
    /// Gets or sets the seleced search checklists in a uri format.
    /// </summary>
    ...
    public virtual CommandContext GetCommandContext()
    {
      ...
      CommandContext context = uri != null ? new CommandContext(Database.GetItem(uri)) : new CommandContext();
      ...
      if (this.RibbonSourceUri != null)
      {
        ...
        if (!Page.ClientScript.IsClientScriptBlockRegistered("load"))
        {
          Page.ClientScript.RegisterStartupScript(this.GetType(), "load", "scCatalog.showRibbon();", true);
        }
      }
    ...
    {
      CommandContext context = new CommandContext { CustomData = this };
    ...
    public void InitializeTextBoxes(IEnumerable<TextBoxDefinition> textBoxDefinitions)
    {
      ...
      foreach (TextBoxDefinition tbd in textBoxDefinitions)
      {
        TextBoxEventArgs args = new TextBoxEventArgs { TextBoxDefinition = tbd };
        this.OnTextBoxCreating(args);

        MaskedTextBox textBox = new MaskedTextBox { ID = TextBoxIdPrefix + args.TextBoxDefinition.Field, CssClass = args.IsWide ? "scSearch Wide" : "scSearch", Text = args.Text, MaskedText = args.TextBoxDefinition.Title, MaskedCssStyle = "scSearchLabel" };
    ...
    public void InitializeChecklists(IEnumerable<ChecklistDefinition> checklists)
    {
      ...
      {
        ChecklistEventArgs args = new ChecklistEventArgs { ChecklistDefinition = checklist };
        ...
        Border header = new Border { ID = Control.GetUniqueID("ChecklistHeader"), InnerHtml = checklist.Header };
        header.Attributes["class"] = "scChecklistHeader";
        border.Controls.Add(header);

        Border checklistBorder = new Border { ID = Control.GetUniqueID("ChecklistBody") };
        ...
        foreach (ChecklistItem checklistItem in checklist.Checkboxes)
        {
          HtmlInputCheckBox cb = new HtmlInputCheckBox { ID = string.Concat(Control.GetUniqueID("checkbox_"), CheckListIdPrefix, checklist.Field), Checked = args.CheckedValues != null && args.CheckedValues.Contains(checklistItem.Value), Value = checklistItem.Value };
          checklistBorder.Controls.Add(cb);

          LiteralControl label = new LiteralControl(string.Format("<label for=\"{0}\">{1}</label><br />", cb.ClientID, checklistItem.Text));
          checklistBorder.Controls.Add(label);

          if (cb.Checked)
          {
            this.AddListStringValue(this.Checklists, checklist.Field, checklistItem.Value);
          }
        }
      }
    }
    ...
    protected virtual void InitializeGrid(IEnumerable<Models.GridColumn> columns, Grid grid)
    {
      ...
      GridLevel level = grid.Levels[0];

      this.GridColumns = new List<Models.GridColumn>(columns);
    ...
    protected override void OnInit(EventArgs e)
    {
      ...
      new SiteIndependentDatabaseSwitcher(this.database);
    ...
    protected virtual void OnAjaxScriptManagerExecute(object sender, AjaxCommandEventArgs args)
    {
      switch (args.Name)
      {
        case "catalog:gridrowdoubleclick":
          GridCommandEventArgs gridArgs = new GridCommandEventArgs { RowID = args.Parameters["rowID"] };
          this.OnGridRowDoubleClick(gridArgs);
          break;
        case "catalog:gridrowselect":
          GridCommandEventArgs gridArgs2 = new GridCommandEventArgs { RowsID = new ListString(args.Parameters["rowIDs"]).ToStringCollection() };
    ...
    {
      List<string> rowValues = e.DataItem as List<string>;
      if (rowValues == null)
      {
        return;
      }
    ...
    /// standard ASP.NET event handler cannot be used to hangle the button click.
    ...
    private NameValueCollection ExtractFields(string prefix)
    {
      ...
      NameValueCollection source = this.Grid.IsCallback ? this.Page.Request.QueryString : this.Page.Request.Form;
      ...
      foreach (string rawKey in source)
      {
        ...
        int i = rawKey.LastIndexOf(prefix);
        if (i < 0)
        {
          continue;
        }
        ...
        string key = rawKey.Substring(i + prefix.Length);
        this.AddListStringValue(fields, key, source[rawKey]);
      }
    ...
    private void AddListStringValue(NameValueCollection list, string key, string value)
    {
      ...
      {
        ListString values = new ListString(list[key]) { value };
