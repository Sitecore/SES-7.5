...
namespace Sitecore.Ecommerce.Web.UI.WebControls
{
  ...
  public class MaskedTextBox : TextBox
  {
    ...
    {
      writer.Write("<span");
      writer.Write(" style=\"position: relative;\"");
      writer.Write(" onfocus=\"this.lastChild.style.zIndex = -1;\" ");
      writer.Write(" onblur=\"this.lastChild.style.zIndex = 'auto';\">");

      base.RenderControl(writer);

      writer.Write("<label for=\"{0}\"", this.UniqueID);
      writer.Write(" class=\"{0}\"", this.MaskedCssStyle);
      writer.Write(" style=\"position: absolute; left: 0; z-index: {0};\">", string.IsNullOrEmpty(this.Text) ? "auto" : "-1");
