  ...
  public abstract class OrderStatusBase : OrderStatus, IEntity
  {
    ...
    public override IEnumerable<OrderStatus> GetAvailableFollowingStatuses()
    {
      if (this.availableFollowingStatuses == null)
      {
        ...
        {
          Sitecore.Data.Items.Item item = Sitecore.Context.Database.GetItem(this.Alias);

          Assert.IsNotNull(item, "item");

          IEnumerable<string> availableStateCodes = (new MultilistField(item.Fields["Available List"])).GetItems().Select(stateItem => stateItem["Code"]);

          foreach (ContainerRegistration registration in Context.Entity.Registrations.Where(r => availableStateCodes.Contains(r.Name)))
          {
            if (registration.RegisteredType == typeof(OrderStatus))
            {
              object entityProvider = Context.Entity.GetType().GetMethod("Resolve").Invoke(Context.Entity, new object[] { typeof(IEntityProvider<>).MakeGenericType(registration.MappedToType), string.Empty, new ResolverOverride[0] });
